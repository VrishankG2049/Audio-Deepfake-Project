...
class SafeEarTrainer(pl.LightningModule):
    def __init__(
        ...
        #pos_weight = torch.tensor([8.8]).to(self.device)

        #self.detect_loss = torch.nn.CrossEntropyLoss(weight=torch.tensor([1.0, 8.8]).to(self.device))
        self.class_weights = torch.tensor([1.0, 8.8])
        self.detect_loss = torch.nn.CrossEntropyLoss(weight=self.class_weights)
    ...
    def forward(self, batch, is_train=True):
        ...
        with torch.no_grad():
            ...
            acoustic_tokens = self.decouple_model(x_wav)
        #raw_logits, raw_feature = self.detect_model([acoustic_tokens])
        # unwrap if tuple
        if isinstance(acoustic_tokens, tuple):
            acoustic_tokens = acoustic_tokens[0]
        _, _, feature, encoder_list = self.decouple_model(x)
        ...
        if is_train:
            ...
            self.detect_loss.weight = self.class_weights.to(raw_logits.device)
            raw_former_loss_ = self.detect_loss(raw_logits, target)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        _, loss, logits, target = self(batch, is_train=False)

        # store for EER (detach to avoid graph issues)
        self.val_index_loader.append(target.detach().cpu())
        self.val_score_loader.append(logits.detach().cpu())

        # ✅ print ONLY current batch labels (correct debugging)
        print("BATCH LABELS:", target.unique())
        print("PRED SAMPLE:", logits[:5].detach().cpu())
