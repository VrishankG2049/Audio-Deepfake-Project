...
def train(cfg: DictConfig, args) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    print("\n[INFO] Running dummy forward test...")

    device = "cuda" if torch.cuda.is_available() else "cpu"

    decouple_model = decouple_model.to(device)

    # Dummy audio (1 sec)
    audio = torch.randn(1, 1, 24000).to(device)

    # Forward pass through decouple model (THIS is SNAC integration)
    o, loss, feature, _ = decouple_model(audio)
